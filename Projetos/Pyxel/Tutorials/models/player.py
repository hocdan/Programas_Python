...
class Snake:
    ...
    #inicializacao dos atributos
    def __init__(self, posX, posY, imgBank, pixelX, pixelY, largura, altura, Dx=8):
        self.posX = posX
        self.posY = posY
        self.imgBank = imgBank
        self.pixelX = pixelX
    ...
    def drawSnake(self, pyxel):
        #desenhando todos os segmentos do corpo da cobra
        for parte in self.corpo:
            #desenhando sprite especifico de acordo com tipo e direcao do segmento
            if (parte.tipo == "head"):
                if (parte.direcao == 'w'):
                    pyxel.blt(parte.posX,
                              parte.posY,
                              self.imgBank,
                              8,
                              0,
    ...
    def grow(self):
        ...
        '''
        cabeca = self.corpo.pop(0)
        parteNova = SnakePart(cabeca.posX, cabeca.posY, "body", cabeca.direcao, 0, 2)
        self.corpo.insert(0, parteNova) #adicionando novo segmento de corpo no antigo lugar da cabeca
        ...
        if (cabeca.direcao == 'w'):
            cabeca.posY -= 8
             ...
             cabeca.posX += 8
        self.corpo.insert(0, cabeca) #reposicionando cabeca de volta ao corpo com atributos atualizados
        #atualizando atributos frente e costa dos antigos segmentos (resto do corpo e cauda)
        for i in range(2, len(self.corpo)):
             #tratando cauda de maneira diferente
            if (i == (len(self.corpo)-1) ):
                self.corpo[i].frente += 1 #atualizar somente frente, costa segue vzia
            else:
                self.corpo[i].frente += 1
                self.corpo[i].costa += 1
